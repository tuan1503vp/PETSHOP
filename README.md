...
2. Dán nội dung cấu hình sau vào và nhập các API key của bạn:
   ```php
   <?php
   // Cấu hình các API key và mật khẩu bảo mật (Không commit file này lên Git)
   ...
   ```
   * *Lưu ý: File `secrets.php` đã được cấu hình trong `.gitignore` để không bao giờ bị đẩy lên GitHub.*
...
## 🔒 Bảo Mật & Đóng Góp
* Không bao giờ thay đổi cấu hình hoặc ghi trực tiếp các API key vào file `app/config/config.php`.
* Mọi khóa kết nối bên thứ ba phải được khai báo trong `app/config/secrets.php`.
* Khi đẩy code mới lên GitHub, hãy đảm bảo chạy lệnh `git status` để kiểm tra các file bí mật đã được ẩn thành công.

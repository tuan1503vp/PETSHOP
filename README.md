...
- **CI/CD:** Deploy tự động thông qua GitHub Actions tới InfinityFree FTP.
...
│   └── workflows/
│       └── deploy.yml        # CI/CD tự động deploy lên FTP Host khi push
...
│   │   └── secrets.php       # Lưu trữ API keys bí mật (không bị theo dõi bởi Git)
│   ├── controllers/          # Bộ điều hướng (Controllers)
│   ├── core/                 # Core Framework (App, Controller, Database)
│   ├── helpers/              # Các hàm bổ trợ (Session, Mailer, Flash, CSRF, XSS)
...
│   ├── uploads/              # Hình ảnh tải lên từ người dùng (sản phẩm, biên lai)
│   └── index.php             # File khởi chạy ứng dụng chính (Front Controller)
├── .gitignore                # Khai báo các file Git cần bỏ qua
├── database.sql              # Cơ sở dữ liệu chính của dự án
...
3. Tạo cơ sở dữ liệu MySQL:
   ...
   - Import file `database.sql` vào cơ sở dữ liệu vừa tạo.
4. Tạo cấu hình bảo mật `secrets.php` (tùy chọn nhưng khuyến khích tại local):
   Tạo file `app/config/secrets.php` để tránh lưu đè API keys trên Git:
   ```php
   <?php
   define('OPENROUTER_API_KEY', 'your_actual_api_key_here');
   // define('SMTP_PASS', 'your_smtp_app_password_here');
   ```

### 3. Trải nghiệm ứng dụng

- Local: Truy cập `http://localhost/PETSHOP/`
- Production: [https://pet.kesug.com/](https://pet.kesug.com/)

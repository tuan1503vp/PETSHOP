        ...
        <div class="bg-red-50 rounded-lg shadow-sm border border-red-200 overflow-hidden">
            <table class="min-w-full divide-y divide-red-200">
                <thead class="bg-red-100/50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-bold text-red-800 uppercase tracking-wider">Sản phẩm</th>
                        <th class="px-6 py-3 text-left text-xs font-bold text-red-800 uppercase tracking-wider">Tồn kho</th>
                        <th class="px-6 py-3 text-left text-xs font-bold text-red-800 uppercase tracking-wider">Tình trạng HSD</th>
                        <th class="px-6 py-3 text-right text-xs font-bold text-red-800 uppercase tracking-wider">Thao tác</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-red-50/30 transition-colors">
                            ...
                            <td class="px-6 py-3 whitespace-nowrap">
                                <?php if ($ep->days_to_expiry < 0): ?>
                                    <span class="px-2.5 py-1 rounded-lg text-xs font-bold border bg-red-100 text-red-800 border-red-200">
                                        Đã hết hạn (<?php echo date('d/m/Y', strtotime($ep->expiry_date)); ?>)
                                    </span>
                                <?php else: ?>
                                    <span class="px-2.5 py-1 rounded-lg text-xs font-bold border bg-amber-100 text-amber-800 border-amber-200 animate-pulse">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="space-y-1">
                                ...
                                <?php if ($product->stock_quantity == 0): ?>
                                    <span class="block px-2 py-0.5 text-[9px] font-black text-red-600 bg-red-50 border border-red-100 rounded-md uppercase tracking-wider w-max">Hết hàng</span>
                                <?php elseif ($product->stock_quantity <= 5): ?>
                                    <span class="block px-2 py-0.5 text-[9px] font-black text-amber-600 bg-amber-50 border border-amber-100 rounded-md uppercase tracking-wider w-max">Sắp hết hàng</span>
                                <?php endif; ?>
                            </div>
                        </td>
                        <!-- Expiry Date Status -->
                            ...
                            <?php if (!empty($product->expiry_date)): ?>
                                <?php 
                                    ...
                                    if ($days < 0) {
                                        ...
                                        $label = "Đã hết hạn (" . date('d/m/Y', strtotime($product->expiry_date)) . ")";
                                    } elseif ($days <= 30) {
                                ...
                                ?>
                                <span class="px-2.5 py-1 rounded-lg text-xs font-bold border <?php echo $badge; ?>">
                                    <?php echo $label; ?>
                                </span>
                            <?php else: ?>

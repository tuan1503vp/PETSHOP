    ...
    <nav class="flex text-sm text-gray-500 mb-8" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                    <i class="fa-solid fa-chevron-right text-xs mx-2"></i>
                    <span class="text-gray-900 font-semibold">Chỉnh sửa</span>
                </div>
            </li>
        </ol>
    </nav>
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-black text-gray-900 flex items-center gap-3">
                    <i class="fa-solid fa-pen-to-square text-primary"></i> Chỉnh sửa: <?php echo htmlspecialchars($data['name']); ?>
                </h1>
                <span class="bg-slate-100 text-slate-700 px-3 py-1 rounded-xl text-xs font-black tracking-wider">
                    <?php echo $data['pet_code']; ?>
                </span>
            </div>
            <p class="text-sm text-gray-500 mt-2">Cập nhật thông tin chi tiết cho bé cưng của bạn.</p>
        </div>

        <form action="<?php echo URLROOT; ?>/pet/edit/<?php echo $data['id']; ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
            <!-- Row 1: Tên & Loại -->
            ...
            <!-- Row 4: Upload ảnh -->
            <div x-data="{ photoName: null, photoPreview: '<?php echo !empty($data['image']) ? URLROOT . '/public/images/' . $data['image'] : ''; ?>' }">
